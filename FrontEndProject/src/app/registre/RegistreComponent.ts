...
export class RegistreComponent implements OnInit {
  ...
  constructor(private httpClient: HttpClient, public messageService: MessageService, public registreService : RegistreService) {
  ...
  ngOnInit(): void {
    ...
    if(!this.dateSelectionner){
      this.dateSelectionner = new Date();
    }
  }
  showSuccess() {
    this.messageService.add({ severity: 'success', summary: 'Sauvegardé avec succés', detail: 'Presence Sauvegardé' });
  }

  showError() {
    this.messageService.add({ severity: 'error', summary: 'Error ', detail: 'presence non sauvegardé' });
  }
  valider(listresult) {
    let dataBaseObject = [];
    // this.ajouterPresence(this.listresult)
    this.registreService.modifiedElements.forEach((object) => {
    ...
    this.httpClient.post<any>('/sauvegarderPresence', dataBaseObject)
      .subscribe(d => {
        this.registreService.modifiedElements = [];
        this.getListregistre();
        this.showSuccess();
      },
        err => {
          this.showError();
  ...
  getRegistre(): Observable<Map<string, RegistreDTO[]>> {
    ...
    return this.httpClient.get<Map<string, RegistreDTO[]>>('/getPresenceByIdEleve', { headers: httpHeader, params: params });
  ...
  getListregistre(): void {
    ...
    //vider le tableau pour une nouvelle selection
    // this.listresult = [];
    //get list nom eleve
    if (this.niveauSelectionner && this.classeSelectionner && this.dateSelectionner){
      this.getListNomEleve().subscribe((listEleve) => {

      let list: RowData[] = [];

      listEleve.forEach(eleve => {
      let rd = { nomEleve: eleve.nomEleve,idEleve:eleve.idEleve };
      list.push(rd);
      });
      this.listresult = list;
      this.listresult = this.listresult.sort((a,b) => a.nomEleve > b.nomEleve ? 1 : -1);
      let hour: any = [];
      let etat: any = [];
      ...
      this.getRegistre().subscribe(result => {
        ...
        map.forEach(nom => {
          let rd = { nomEleve: nom, idEleve:null }
        ...
        list.forEach((eleve) => {
          rowDataList.forEach((rowData) => {
              if(rowData.idEleve === eleve.idEleve) {
                eleve = rowData;
                return;
              }
          });
          listFinal.push(eleve);
        });
        this.listresult = listFinal;
        this.listresult = this.listresult.sort((a,b) => a.nomEleve > b.nomEleve ? 1 : -1);
      });
  ...
  getListNomEleve(): Observable<EleveNoteDTO[]> {
    ...
    let params: HttpParams = new HttpParams();
    params = params.append('idClasse', this.classeSelectionner.toString());
    return this.httpClient.get<EleveNoteDTO[]>('/getListEleveByIdClasse', { params: params, headers: httpHeader });
  }

  getNiveau(): Observable<Niveau[]> {
    let httpHeader: HttpHeaders = new HttpHeaders();
    httpHeader = httpHeader.set('Content-Type', 'application/json; charset=utf-8');
    return this.httpClient.get<Niveau[]>('/getListNiveau?idNiveau= 1,2,3', { headers: httpHeader, withCredentials: true },);
  ...
  getListNiveau(): void {
    this.getNiveau().subscribe(result => {
      ...
      this.niveauSelectionner = this.niveauList[0].idNiveau;
      this.getListClasse();
    });
  }
  ...
  getClasseByNiveau(): Observable<Classe[]> {
    ...
    return this.httpClient.get<Classe[]>('/getClassesByIdNiveau', { headers: httpHeader, params: niveauParametre })
  }
  getListClasse(): void {
    this.getClasseByNiveau().subscribe(result => {
      this.classeList = result;
      if (result && result.length > 0 ){
        this.classeSelectionner = result[0].idClasse;
        this.getListregistre();
      } 
  ...
  ajouterPresence(dataBaseObject: any): Observable<any> {
    ...
    return this.httpClient.post<any>('/sauvegarderPresence', dataBaseObject)
  }
